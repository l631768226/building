...
public interface TraceDataDao {
    ...
    @Select("select * from traceData where md5 = #{md5}")
    TraceData findByMd5(@Param("md5") String md5);


    @Insert("insert into traceData (md5, pathList, pointList, userPath, preData) values " +
    ...
    @Update("update traceData set proData = #{proData}, deeledPathData = #{deeledPathData}, deeledPointData = #{deeledPointData} " +
            "where md5 = #{md5}")
